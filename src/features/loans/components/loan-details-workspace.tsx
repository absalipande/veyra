...
function InstallmentStatusChip({ status }: { status: "paid" | "current" | "upcoming" }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-[0.66rem] font-semibold uppercase tracking-[0.1em] ${className}`}>
      {label}
    </span>
  );
}
...
export function LoanDetailsWorkspace({ loanId }: { loanId: string }) {
  ...
  const paidInstallments = loan.paidInstallmentCount ?? timeline.filter((entry) => entry.status === "paid").length;
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h1 className="text-[1.35rem] font-semibold tracking-tight text-foreground sm:text-[1.55rem]">
                  {loan.name}
                </h1>
                <span
                ...
                <div className="relative h-14 w-14 shrink-0">
                  <svg className="h-14 w-14 -rotate-90" viewBox="0 0 48 48" aria-hidden="true">
                    ...
                    <circle
                      ...
                      strokeDasharray={`${(Math.max(Math.min(progressPercent, 100), 0) / 100) * 113.1} 113.1`}
                    />
                  </svg>
          ...
          <Card className="border-border/70 bg-card/90">
            ...
            <CardContent className="space-y-2 text-[0.9rem]">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="font-semibold text-foreground">{formatCurrencyMiliunits(loanAmount, loan.currency)}</span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span className="text-muted-foreground">Total payable</span>
                <span className="font-semibold text-foreground">{formatCurrencyMiliunits(loan.totalPayable, loan.currency)}</span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span className="text-muted-foreground">Finance charge</span>
                <span className="font-semibold text-foreground">{formatCurrencyMiliunits(loan.financeCharge, loan.currency)}</span>
              </div>
              <div className="flex items-center justify-between gap-3">
