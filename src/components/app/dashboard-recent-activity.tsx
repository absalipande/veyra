    ...
    <section className="space-y-4 pb-24 md:space-y-6 md:pb-0">
      ...
      <div className="grid gap-4 md:grid-cols-2 md:gap-6">
        <Card className="rounded-[1.5rem] border-white/80 bg-white/84 shadow-[0_26px_80px_-60px_rgba(10,31,34,0.35)] dark:border-white/8 dark:bg-[#182123] dark:shadow-[0_28px_90px_-60px_rgba(0,0,0,0.6)]">
          <CardContent className="p-4 sm:p-5 md:p-6 lg:p-7">
            <div className="mb-4 flex items-center justify-between gap-3 md:mb-5">
              <h3 className="text-[1.25rem] font-semibold tracking-tight text-[#10292B] dark:text-foreground md:text-[1.2rem] lg:text-[1.3rem]">Important accounts</h3>
              <Button asChild variant="outline" size="sm" className="h-10 rounded-full px-4 text-[0.86rem] font-medium md:h-9 md:px-3.5 md:text-[0.8rem]">
                <Link href="/accounts">View all</Link>
              </Button>
            </div>

            <div className="overflow-hidden rounded-[1.1rem] border border-border/70 md:rounded-[1.2rem]">
              <div className="divide-y divide-border/70">
                {importantAccounts.length === 0 ? (
                  ...
                  importantAccounts.map((account) => {
                    ...
                    return (
                      <div key={account.id} className="flex items-center gap-3 px-4 py-3.5 md:gap-3.5 md:px-5 md:py-3.5">
                        <div className={`flex size-11 shrink-0 items-center justify-center rounded-full text-[0.8rem] font-semibold ${institution.tone} md:size-10 md:text-[0.76rem]`}>
                          {institution.initials}
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-[1rem] font-semibold tracking-tight text-[#10292B] dark:text-foreground md:text-[0.98rem]">{account.name}</p>
                          <div className="mt-1 flex items-center gap-2 text-[0.78rem] leading-none md:text-[0.86rem]">
                            {account.note !== "Active" ? (
                        ...
                        <div className="shrink-0 text-right">
                          ...
                          <div className={`mt-1 flex items-center justify-end gap-1.5 text-[0.76rem] ${account.amountMetaTone} md:text-[0.78rem]`}>
        ...
        <Card className="rounded-[1.5rem] border-white/80 bg-white/84 shadow-[0_26px_80px_-60px_rgba(10,31,34,0.35)] dark:border-white/8 dark:bg-[#182123] dark:shadow-[0_28px_90px_-60px_rgba(0,0,0,0.6)]">
          <CardContent className="p-4 sm:p-5 md:p-6 lg:p-7">
            <div className="mb-4 flex items-center justify-between gap-3 md:mb-5">
              <h3 className="text-[1.25rem] font-semibold tracking-tight text-[#10292B] dark:text-foreground md:text-[1.2rem] lg:text-[1.3rem]">Recent movement</h3>
              <Button asChild variant="outline" size="sm" className="h-10 rounded-full px-4 text-[0.86rem] font-medium md:h-9 md:px-3.5 md:text-[0.8rem]">
              ...
              <div className="divide-y divide-border/70">
                {transactions.slice(0, 5).length === 0 ? (
                  ...
                  transactions.slice(0, 5).map((event) => {
                    ...
                    return (
                      <div key={event.id} className="flex items-center gap-3 px-4 py-3.5 md:gap-3.5 md:px-5 md:py-3.5">
                        <div className={`flex size-11 shrink-0 items-center justify-center rounded-full ${iconTone} md:size-10`}>
                          {event.type === "transfer" ? (
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-[1rem] font-semibold tracking-tight text-[#10292B] dark:text-foreground md:text-[0.98rem]">
                            {event.description}
                          </p>
                          <p className="mt-0.5 text-[0.8rem] text-muted-foreground md:text-[0.8rem]">{getMovementTypeLabel(event.type)}</p>
                        </div>
                        <div className="shrink-0 text-right">
                          <p className={`text-[1rem] font-semibold tracking-tight ${amountPositive ? "text-emerald-700 dark:text-emerald-300" : "text-rose-700 dark:text-rose-300"} md:text-[0.98rem]`}>
                            {amountPositive ? "+" : "-"}
                            {formatCurrencyMiliunits(Math.abs(signedAmount), event.currency)}
                          </p>
                          <p className="mt-0.5 text-[0.8rem] text-muted-foreground md:text-[0.8rem]">
      ...
      <Card className="rounded-[1.5rem] border-white/80 bg-white/84 shadow-[0_26px_80px_-60px_rgba(10,31,34,0.35)] dark:border-white/8 dark:bg-[#182123] dark:shadow-[0_28px_90px_-60px_rgba(0,0,0,0.6)]">
        <CardContent className="p-4 sm:p-5 md:p-6 lg:p-7">
          <div className="mb-4 flex items-center justify-between gap-3 md:mb-5">
            <h3 className="text-[1.25rem] font-semibold tracking-tight text-[#10292B] dark:text-foreground md:text-[1.2rem] lg:text-[1.3rem]">Budget posture</h3>
            <Button asChild variant="outline" size="sm" className="h-10 rounded-full px-4 text-[0.86rem] font-medium md:h-9 md:px-3.5 md:text-[0.8rem]">
              <Link href="/budgets">{budgetPosture.action}</Link>
            </Button>
          </div>

          <div className="rounded-[1.1rem] border border-border/70 bg-background/70 p-4 md:flex md:items-center md:justify-between md:gap-4 md:p-5 dark:bg-[#141d1f]">
            <div className="flex items-center gap-3">
              <div className="flex size-12 items-center justify-center rounded-xl bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-200 md:size-12 md:rounded-[0.9rem]">
                <PiggyBank className="size-6 md:size-6" />
              </div>
              <div>
                <p className="text-[1rem] font-semibold tracking-tight text-[#10292B] dark:text-foreground md:text-[0.98rem]">{budgetPosture.title}</p>
                <p className="mt-1 text-[0.9rem] text-muted-foreground md:text-[0.88rem]">{budgetPosture.body}</p>
              </div>
            </div>
          </div>
        </CardContent>
